...
- Primary auth is at the **edge**: the Kubernetes deploy ([deploy/helm/datametl](deploy/helm/datametl)) ships **oauth2-proxy** in front of the app for **Keycloak/OIDC SSO**. There is now ALSO an **optional simple in-app login** for running the app directly (outside k8s) without Keycloak — a single shared username/password, **off by default**, gated by `AUTH_ENABLED` (see [backend/app/auth.py](backend/app/auth.py), [backend/app/api/auth.py](backend/app/api/auth.py), and the `AuthMiddleware` in [backend/app/main.py](backend/app/main.py)). It carries a stdlib-signed **bearer token** (no cookies; works in dev cross-origin and prod same-origin), and the credential is scrypt-hashed + Fernet-encrypted in `app_settings`. **Leave `AUTH_ENABLED=false` when behind oauth2-proxy** to avoid a double login. Multi-user accounts + viewer/operator RBAC + audit remain a future phase.
